# app_ar.py
# واجهة عربية بالكامل + RTL + ألوان، وتدعم PDF / TXT / DOCX كما هو مطلوب.
...
DEFAULT_TOPK = 5
DEFAULT_MAXLEN = 256  # طابقيه مع قيمة التدريب لديك
...
with t1:
    ...
    def run_predict_ar(raw_text: str):
    ...
    if btn_file:
        if file is None:
            ...
            try:
                raw_text = load_file_text(file)  # ✅ يدعم PDF/TXT/DOCX
...
with t3:
    ...
    if go:
        ...
        for f in files or []:
            try:
                texts.append(load_file_text(f))  # ✅ يدعم PDF/TXT/DOCX
            except Exception as e:
